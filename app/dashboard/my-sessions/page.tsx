...
export default function Page() {
	...
	const [pastSlots, setPastSlots] = useState<ISlotPopulated[]>([]);
	...
	const fetchSlots = async (userId: string, authToken: string) => {
		try {
			const res = await FetchApi.get(
				`/slot?userId=${userId}&populated=true`,
	...
	const fetchPastSlots = async (userId: string, authToken: string) => {
		try {
			const res = await FetchApi.get(
				`/slot?past=true&userId=${userId}&order=desc&populated=true`,
				{ headers: { Authorization: `Bearer ${authToken}` } },
			);
	...
	async function handleCancel(slot: ISlotPopulated) {
		...
		try {
			...
			if (res?.data.success === true) {
				...
				// Close modal first
				setModalSlot(null); // Refresh slots after a brief delay to ensure alert is visible
				setTimeout(() => {
					fetchSlots(userId, token);
					fetchPastSlots(userId, token);
				}, 100);
					...
					<div className="mt-12">
						...
						<div className="flex flex-wrap justify-center gap-4 md:gap-6">
							{pastSlots.map((slot) => (
								<div
									key={slot._id + "-past"}
									className="w-full md:w-80 bg-white border border-gray-200 rounded-xl shadow-sm opacity-80 overflow-hidden"
								>
									{/* Card Content */}
									<div className="p-6">
										{/* Program Badge */}
